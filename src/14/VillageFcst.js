...
export default function VillageFcst() {
  const dt = useParams().dt;
  const area = useParams().area;
  const x = useParams().x;
  const y = useParams().y;

  const [villageDt, setVillageDt] = useState();
  ...
  const getData = async (url) => {
    ...
    setVillageDt(data.response.body.items.item);
  };
  ...
  useEffect(() => {
    ...
    let tmp = villageDt.filter(item => item['category'] === selItem).map((item) => (
